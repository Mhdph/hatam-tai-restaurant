...
import { deleteAdditionalFn, getAllAdditionalFn } from "../../../config";
import { FoodD } from "../../../types";
import AddAditional from "../Modal/AddAditional";
...
function Additional() {
  ...
  const { isLoading, data, error } = useQuery({
    queryKey: ["foodall"],
    queryFn: getAllAdditionalFn,
  });
  const { mutate: deleteCategory } = useMutation(
    (Id: string) => deleteAdditionalFn(Id),
    {
      onSuccess(data) {
  ...
  return (
    <div className="overflow-x-auto p-4 capitalize">
      <AddAditional open={open} setOpen={setOpen} />
              ...
              .map((item: FoodD) => (
                <tr className="border-b bg-white">
                  ...
                  <td className="py-4 px-6">
                    <button
                      onClick={() => navigate(`/panel/additonal/${item._id}`)}
                      type="button"
